...
protected:
  void configure() override
  {
    ...
    subscribe<std::string>("alert",
                           [](const std::string& msg) { std::cout << "[AlertNode] 🚨 ALERT: " << msg << std::endl; });
  }
};
...
int main()
{
  ...
  // 5b: ThreadPool - фиксированное количество потоков
  {
    ...
    ExecutionManager em(ExecutionManager::Mode::RealTime, nodes, ExecutionManager::ThreadingMode::ThreadPool,
  ...
  // 5d: SingleThreaded - все узлы на одном потоке
  {
    ...
    ExecutionManager em(ExecutionManager::Mode::RealTime, nodes, ExecutionManager::ThreadingMode::SingleThreaded);
    em.startAll();
    std::this_thread::sleep_for(std::chrono::milliseconds(200));
    em.stopAll();
  }
  ...
  std::cout << "\n--- Test 6: Priorities and Statistics ---" << std::endl;
  {
    ...
    ExecutionManager em(ExecutionManager::Mode::RealTime, nodes, ExecutionManager::ThreadingMode::ThreadPool, 2);
