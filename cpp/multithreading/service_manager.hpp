...
public:
        ...
        if (mode_ == Mode::RealTime)
        {
            if (threading_mode_ == ThreadingMode::ThreadPool)
            {
                ...
                if (thread_pool_size == 0)
                {
                    ...
                    if (thread_pool_size == 0) thread_pool_size = 4;  // fallback
                }
    ...
    void printStats() const
    {
        ...
        internal::log_info("Total: " + std::to_string(total_messages) + " messages, " + 
                    ...
                    std::function<void(const void*)> deliver) override
    {
        ...
        for (const auto& sub : it->second)
        {
            ...
            auto& ctx = ctx_it->second;
            {
                ...
                ctx.queue.push([cb = sub.cb, data_ptr]() { 
                    cb(data_ptr.get());
                });
            }
...
private:
    ...
    void runService(const ServicePtr& Service)
    {
        ...
        while (ctx.running)
        {
            ...
            ctx.cv.wait_for(lk, timeout, [&ctx]() {
                return !ctx.queue.empty() || !ctx.running;
            });
        }
    }
    ...
    void workerThreadLoop()
    {
        ...
        while (worker_threads_running_)
        {
            ...
            for (auto& kv : contexts_)
            {
                ...
                switch (priority)
                {
                    case Service::Priority::Critical: skip_factor = 1; break;
                    case Service::Priority::High:     skip_factor = 1; break;
                    case Service::Priority::Normal:   skip_factor = 2; break;
                    case Service::Priority::Low:      skip_factor = 4; break;
                }
            ...
            if (!work_done)
            {
                ...
                work_available_cv_.wait_for(lk, std::chrono::microseconds(100), [this]() {
                    return !worker_threads_running_;
                });
            }
        }
    }
