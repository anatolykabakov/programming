...
public:
    ...
    void subscribe(const std::string& topic, Node* node, std::function<void(const T&)> cb)
    {
        ...
        subscriptions_[topic].push_back(
            {node, [cb](const void* data) { cb(*static_cast<const T*>(data)); }});
    }
...
inline size_t Node::getQueueSize() const
{
    return em_->getQueueSize(const_cast<Node*>(this));
}

inline uint64_t Node::now() const
{
    return em_->now();
}
