...
def print_profile_data(aggregated_data: Dict[str, float]) -> None:
    ...
    for checker_name, data in sorted_checkers:
        ...
        combined_percent = (combined_time / total_combined * 100) if total_combined > 0 else 0
        ...
        print_stderr(f"   {user_str}   {sys_str}   {combined_str}   {wall_str}  {checker_name}")
...
def _rehost_path(path: str, old_root: str, new_root: str) -> str:
    if path == old_root:
        return new_root
...
def _rehost_compiler_arg(token: str, old_root: str, new_root: str) -> str:
    """Re-map only full path prefixes (old_root/...), not /src inside .../app/src/..."""
    if not token:
        return token
    if token.startswith(old_root + "/") or token == old_root:
        return new_root + token[len(old_root) :]
    for flag in ("-I", "-isystem", "-iquote", "-idirafter"):
        if token.startswith(flag) and len(token) > len(flag):
            rest = token[len(flag) :]
            if rest.startswith(old_root + "/") or rest == old_root:
                return flag + new_root + rest[len(old_root) :]
    return token


def _rehost_command_string(command: str, old_root: str, new_root: str) -> str:
    try:
        parts = shlex.split(command, posix=True)
    except ValueError:
        return command
    parts = [_rehost_compiler_arg(p, old_root, new_root) for p in parts]
    return subprocess.list2cmdline(parts)


def rehost_compile_commands(entries: List[Dict[str, Any]], old_root: str, new_root: str) -> None:
    """Rewrite Docker (or other) source root in compile_commands.json entries in place."""
    for e in entries:
        for key in ("directory", "file", "output"):
            if key in e and isinstance(e[key], str):
                e[key] = _rehost_path(e[key], old_root, new_root)
        cmd = e.get("command")
        if isinstance(cmd, str):
            e["command"] = _rehost_command_string(cmd, old_root, new_root)
        args = e.get("arguments")
        if isinstance(args, list):
            e["arguments"] = [
                (_rehost_compiler_arg(a, old_root, new_root) if isinstance(a, str) else a)
                for a in args
            ]


def prepare_compilation_db_for_host(
    database: List[Dict[str, Any]], build_path_abs: str, hide_progress: bool
) -> Tuple[str, List[Dict[str, Any]], Optional[str]]:
    """If compile_commands uses paths under a Docker mount that do not exist here,
    rewrite them to the repo root (parent of build/) and return a temp dir for -p.

    Override the Docker root with env COMPILE_COMMANDS_DOCKER_ROOT (default /src).
    """
    if not database:
        return build_path_abs, database, None
    sample_dir = database[0].get("directory", "")
    if os.path.isdir(sample_dir):
        return build_path_abs, database, None
    repo_root = os.path.dirname(build_path_abs)
    docker_root = os.environ.get("COMPILE_COMMANDS_DOCKER_ROOT", "/src").rstrip("/")
    if sample_dir != docker_root and not sample_dir.startswith(docker_root + os.sep):
        return build_path_abs, database, None
    rehost_compile_commands(database, docker_root, repo_root)
    tmp = tempfile.mkdtemp(prefix="run-clang-tidy-compdb-")
    with open(os.path.join(tmp, "compile_commands.json"), "w") as f:
        json.dump(database, f, indent=2)
    if not hide_progress:
        print(
            f"compile_commands.json uses container paths ({docker_root!r}); "
            f"rewrote to {repo_root!r} for clang-tidy (-p {tmp})",
            file=sys.stderr,
        )
    return tmp, database, tmp
...
async def main() -> None:
    ...
    db_file = os.path.join(build_path, db_path)
    if not os.path.isfile(db_file):
        print(
            f"error: compilation database missing: {db_file}\n"
            "Fix: from the repo root, run CMake with "
            "-DCMAKE_EXPORT_COMPILE_COMMANDS=ON and build once "
            "(e.g. ./build.sh or ./scripts/build_cpp.sh).",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    tidy_build_path, database, compdb_tmp = prepare_compilation_db_for_host(
        database, build_path, args.hide_progress
    )
    if compdb_tmp:
        atexit.register(lambda d=compdb_tmp: shutil.rmtree(d, ignore_errors=True))
