...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        dest="only_in_compile_db",
        help="Skip files not present in the compilation database.",
    )
    ...
    if args.only_in_compile_db and args.build_path:
        db_path = os.path.join(args.build_path, "compile_commands.json")
        if os.path.isfile(db_path):
            with open(db_path, "r") as f:
                compile_db = json.load(f)
            compile_db_files = {os.path.normpath(entry["file"]) for entry in compile_db}
            filtered = {}
            for name, lines in lines_by_file.items():
                if (
                    not name.endswith(".cpp")
                    or os.path.normpath(os.path.abspath(name)) in compile_db_files
                ):
                    filtered[name] = lines
                else:
                    print("Skipping %s (not in compilation database)" % name)
            lines_by_file = filtered
        else:
            print("Warning: compile_commands.json not found at %s" % db_path)
