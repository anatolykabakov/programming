...
DEFAULT_MAP = SIM_DIR / "map_large.png"
...
def Draw(img_map, scale, bot_pos, sensor_data, bot_param):
    ...
    cv2.circle(
        img, (int(scale * bot_pos[0]), int(scale * bot_pos[1])), int(3 * scale), (0, 0, 255), -1
    )
    return img
...
def AdaptiveGetMap(gmap):
    mimg = gmap.GetMapProb(
        gmap.boundary[0] - 20, gmap.boundary[1] + 20, gmap.boundary[2] - 20, gmap.boundary[3] + 20
    )
...
if __name__ == "__main__":
    ...
    env = SingleBotLaser2Dgrid(bot_pos, bot_param, str(DEFAULT_MAP))
