...
const double FOCAL_LENGTH =
    4308 / IMAGE_DOWNSAMPLE;      // focal length in pixels, after downsampling, guess from jpeg EXIF data
...
const std::vector<std::string> IMAGES = {"DSC02638.JPG", "DSC02639.JPG", "DSC02640.JPG", "DSC02641.JPG",
                                         "DSC02642.JPG"};
...
int main(int argc, char* argv[])
{
  ...
  // Recover motion between previous to current image and triangulate points
  {
    ...
    // Setup camera matrix
    double cx = config.calibration.intrinsic.cx;  // SFM.img_pose[0].img.size().width/2;
    double cy = config.calibration.intrinsic.cx;  // SFM.img_pose[0].img.size().height/2;

    Point2d pp(cx, cy);

    Mat K = Mat::eye(3, 3, CV_64F);

    K.at<double>(0, 0) = config.calibration.intrinsic.fx;
    K.at<double>(1, 1) = config.calibration.intrinsic.fy;
    K.at<double>(0, 2) = config.calibration.intrinsic.cx;
    K.at<double>(1, 2) = config.calibration.intrinsic.cy;
  ...
  gtsam::Values result;
  {
    ...
    for (size_t i = 0; i < SFM.img_pose.size(); i++) {
      ...
      Rot3 R(img_pose.T.at<double>(0, 0), img_pose.T.at<double>(0, 1), img_pose.T.at<double>(0, 2),

             img_pose.T.at<double>(1, 0), img_pose.T.at<double>(1, 1), img_pose.T.at<double>(1, 2),

             img_pose.T.at<double>(2, 0), img_pose.T.at<double>(2, 1), img_pose.T.at<double>(2, 2));
      ...
      if (i == 0) {
        noiseModel::Diagonal::shared_ptr pose_noise =
      ...
      for (size_t k = 0; k < img_pose.kp.size(); k++) {
        if (img_pose.kp_3d_exist(k)) {
          ...
          if (SFM.landmark[landmark_id].seen >= MIN_LANDMARK_SEEN) {
            ...
            graph.emplace_shared<GeneralSFMFactor2<Cal3_S2>>(pt, measurement_noise, Symbol('x', i),
                                                             Symbol('l', landmark_id), Symbol('K', 0));
          }
        }
      }
    }
    ...
    noiseModel::Diagonal::shared_ptr cal_noise =
        noiseModel::Diagonal::Sigmas((Vector(5) << 100, 100, 0.01 /*skew*/, 100, 100).finished());
