...
int main()
{
  const std::string sInputDir = "/atom/programming/robotics/sfm/data/";
  ...
  {
    Features2SVG(jpg_filenameL, {imageL.Width(), imageL.Height()}, regionsL->Features(), jpg_filenameR,
                 {imageR.Width(), imageR.Height()}, regionsR->Features(), "02_features.svg");
  }
  ...
  //-- Perform matching -> find Nearest neighbor, filtered with Distance ratio
  {
    ...
    Matches2SVG(jpg_filenameL, {imageL.Width(), imageL.Height()}, regionsL->GetRegionsPositions(), jpg_filenameR,
                {imageR.Width(), imageR.Height()}, regionsR->GetRegionsPositions(), vec_PutativeMatches,
                "03_Matches.svg", bVertical);
  }
  ...
  // Essential geometry filtering of putative matches
  {
    Mat3 K;
    // read K from file
    fs::path intrinsicPath = fs::path(sInputDir) / "K.txt";
    if (!readIntrinsic(intrinsicPath.string(), K)) {
      std::cerr << "Cannot read intrinsic parameters." << std::endl;
      return EXIT_FAILURE;
    }

    const Pinhole_Intrinsic camL(imageL.Width(), imageL.Height(), K(0, 0), K(0, 2), K(1, 2)),
    ...
    std::cout << "\nFound an Essential matrix:\n"
              ...
              << "\t#matches: " << vec_PutativeMatches.size() << std::endl;

    // Show Essential validated point
    const bool bVertical = true;
    InlierMatches2SVG(jpg_filenameL, {imageL.Width(), imageL.Height()}, regionsL->GetRegionsPositions(), jpg_filenameR,
                      {imageR.Width(), imageR.Height()}, regionsR->GetRegionsPositions(), vec_PutativeMatches,
                      relativePose_info.vec_inliers, "04_ACRansacEssential.svg", bVertical);
    ...
    switch (iBAType) {
      case 1:  // Each view use it's own pinhole camera intrinsic
        tiny_scene.intrinsics[0].reset(
            new Pinhole_Intrinsic(imageL.Width(), imageL.Height(), K(0, 0), K(0, 2), K(1, 2)));
        tiny_scene.intrinsics[1].reset(
            new Pinhole_Intrinsic(imageR.Width(), imageR.Height(), K(0, 0), K(0, 2), K(1, 2)));
        break;
      ...
      case 3:  // Shared pinhole camera intrinsic with radial K3 distortion
        tiny_scene.intrinsics[0].reset(
            new Pinhole_Intrinsic_Radial_K3(imageL.Width(), imageL.Height(), K(0, 0), K(0, 2), K(1, 2)));
        break;
      default:
    ...
    for (const auto inlier_idx : relativePose_info.vec_inliers) {
      ...
      if (Triangulate2View(pose0.rotation(), pose0.translation(),
                           (*tiny_scene.intrinsics[0])(LL.coords().cast<double>()), pose1.rotation(),
                           pose1.translation(), (*tiny_scene.intrinsics[1])(RR.coords().cast<double>()), X,
                           triangulation_method)) {
        // Add a new landmark (3D point with it's 2d observations)
        Landmark landmark;
        landmark.obs[tiny_scene.views[0]->id_view] =
            Observation(LL.coords().cast<double>(), vec_PutativeMatches[inlier_idx].i_);
        landmark.obs[tiny_scene.views[1]->id_view] =
            Observation(RR.coords().cast<double>(), vec_PutativeMatches[inlier_idx].j_);
        landmark.X = X;
    ...
    for (const auto& [index, pose] : tiny_scene.poses) {
    ...
    bundle_adjustment_obj.Adjust(tiny_scene, Optimize_Options(Intrinsic_Parameter_Type::ADJUST_ALL,
