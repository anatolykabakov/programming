...
const std::map<std::string, std::string> DEFAULT_SUB_TOPICS = {
    {"can/tx", "tcp://127.0.0.1:5564"},                 // Joystick CAN commands
...
const std::map<std::string, std::string> DEFAULT_PUB_TOPICS = {
    {"can/rx", "tcp://127.0.0.1:5563"},        // CAN RX data to Java
    {"panda/health", "tcp://127.0.0.1:5565"},  // Panda health status to Java
};
...
private:
  ...
  void onInternalMessage(const std::string& topic_name, const ai::flow::android::ZMQMessage& msg);
