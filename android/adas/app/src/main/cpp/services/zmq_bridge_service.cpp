...
void ZmqBridgeService::onInternalMessage(const std::string& topic_name, const ai::flow::android::ZMQMessage& msg)
{
  ...
  for (const auto& [topic_name, endpoint] : pub_topics_) {
    try {
      ...
      subscribe<ai::flow::android::ZMQMessage>(
          topic_name,
          [this, topic_name](const ai::flow::android::ZMQMessage& msg) { onInternalMessage(topic_name, msg); });
