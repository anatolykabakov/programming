...
TEST(ZMQIMUTest, StartAdasAppAndConnect) {
    ...
    zmq_msg.set_timestamp(std::chrono::duration_cast<std::chrono::milliseconds>(
    ...
    LOGI("Sent protobuf IMU message to AdasApp: accel(%.1f, %.1f, %.1f), gyro(%.1f, %.1f, %.1f)", 
    ...
    LOGI("Received IMU data: accel(%.1f, %.1f, %.1f), gyro(%.1f, %.1f, %.1f), mag(%.1f, %.1f, %.1f)", 
         received_imu.accel_x(), received_imu.accel_y(), received_imu.accel_z(),
         received_imu.gyro_x(), received_imu.gyro_y(), received_imu.gyro_z(),
         received_imu.mag_x(), received_imu.mag_y(), received_imu.mag_z());
