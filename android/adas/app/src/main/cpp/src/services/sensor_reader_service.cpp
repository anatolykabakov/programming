...
void SensorReaderService::configure()
{
  ...
  subscribe<ai::flow::adas::ZMQMessage>("sensors/imu",
                                        [this](const ai::flow::adas::ZMQMessage& msg) { processImuData(msg); });

  subscribe<ai::flow::adas::ZMQMessage>("sensors/gps/location",
                                        [this](const ai::flow::adas::ZMQMessage& msg) { processGpsLocation(msg); });

  subscribe<ai::flow::adas::ZMQMessage>("sensors/gps/data",
                                        [this](const ai::flow::adas::ZMQMessage& msg) { processGpsData(msg); });

  subscribe<ai::flow::adas::ZMQMessage>("sensors/camera/image",
                                        [this](const ai::flow::adas::ZMQMessage& msg) { processCameraBuffer(msg); });
...
void SensorReaderService::processImuData(const ai::flow::adas::ZMQMessage& msg)
{
...
void SensorReaderService::processGpsLocation(const ai::flow::adas::ZMQMessage& msg)
{
  LOGI("Received gpsLocation message");
  logSensorData(msg);
}

void SensorReaderService::processGpsData(const ai::flow::adas::ZMQMessage& msg)
{
  LOGI("Received gpsData message");
  logSensorData(msg);
}

void SensorReaderService::processCameraBuffer(const ai::flow::adas::ZMQMessage& msg)
{
  LOGI("Received wideRoadCameraBuffer message");
  logSensorData(msg);
}

void SensorReaderService::logSensorData(const ai::flow::adas::ZMQMessage& message)
{
