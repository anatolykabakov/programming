...
void LaneKeepService::configure()
{
  ...
  LOGI("LaneKeepService: subscribed %s + %s → %s", topics::kVehicleChassis, topics::kVisionPath, topics::kLaneKeep);
}
...
void LaneKeepService::publishOutputs(const LaneKeepOutput& out)
{
  ...
  // Torque command for Panda CarController (same topic as Android inject).
  ...
  cmd->set_torque_cnm(steer_output_enabled_ ? torque : 0);
  cmd->set_enabled(steer_output_enabled_ && out.has_target && out.status == "ok");
...
LaneKeepOutput LaneKeepService::step(double speed_mps, const std::vector<Vec2>& polyline_ego)
{
  ...
  if (max_steer_rad_ > 1e-6) {
    out.steer_norm = std::clamp(out.steer_rad / max_steer_rad_, -1.0, 1.0);
  }
  out.status = "ok";
  last_ = out;
  return out;
