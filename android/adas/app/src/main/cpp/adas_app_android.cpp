...
JNIEXPORT void JNICALL Java_ai_flow_android_AdasAppHandler_nativeStart(JNIEnv* env, jclass cls) {
    LOGI("JNI nativeStart called");
    try {
        if (!adas_app) {
            LOGI("Creating new AdasApp instance");
        ...
        if (result) {
            LOGI("AdasApp started successfully via JNI");
        } else {
            LOGE("AdasApp failed to start via JNI");
        }
