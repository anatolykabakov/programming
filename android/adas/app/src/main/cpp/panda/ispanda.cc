        ...
        if (panda_devices.empty()) {
            ...
            while (panda->connected() && i > 0 && panda->comms_healthy()) {
                ...
                if (!raw_can_data.empty()) {
                    std::cout << "   Can data: size " << raw_can_data.size() 
                              << " addr: 0x" << std::hex << std::uppercase << raw_can_data[0].address 
                              << " busTime: " << std::dec << raw_can_data[0].busTime 
            ...
            // Статистика сессии
            std::cout << "Session completed - Total frames: " << total_frames 
                      << ", ESP_19 frames: " << esp19_frames << std::endl;
        }
