...
public:
    ...
    void logCanFrame(const can_frame& frame) {
        ...
        log_file_ << getCurrentTimestamp() << ","
                  ...
                  << std::dec << frame.busTime << ","
                  << static_cast<int>(frame.src) << ","
                  << static_cast<int>(frame.dat.size()) << ","
                  << formatHexData(frame.dat) << std::endl;
    ...
    static std::optional<CanFrameWithTimestamp> parseCanFrameFromLine(const std::string& log_line) {
        ...
        if (!std::getline(line_stream, timestamp, ',')) return std::nullopt;
        if (!std::getline(line_stream, address, ',')) return std::nullopt;
        if (!std::getline(line_stream, bus_time, ',')) return std::nullopt;
        if (!std::getline(line_stream, src, ',')) return std::nullopt;
        if (!std::getline(line_stream, length, ',')) return std::nullopt;
        if (!std::getline(line_stream, data)) return std::nullopt;
