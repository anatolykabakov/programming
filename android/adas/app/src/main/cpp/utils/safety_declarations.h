...
#define GET_FLAG(value, mask)                                                                                          \
  (((__typeof__(mask))(value) & (mask)) == (mask))  // cppcheck-suppress misra-c2012-1.2; allow __typeof__

#define BUILD_SAFETY_CFG(rx, tx)                                                                                       \
  ((safety_config){(rx), (sizeof((rx)) / sizeof((rx)[0])), (tx), (sizeof((tx)) / sizeof((tx)[0])), false})
...
typedef struct {
  ...
  unsigned int bus;
  int len;
  bool check_relay;  // if true, trigger relay malfunction if existence on destination bus and block forwarding to
                     // destination bus
  bool disable_static_blocking;  // if true, static blocking is disabled so safety mode can dynamically handle it (e.g.
                                 // selective AEB pass-through)
} CanMsg;
...
typedef struct {
  ...
  bool msg_seen;
  int index;  // if multiple messages are allowed to be checked, this stores the index of the first one seen. only
              // msg[msg_index] will be used
...
bool steer_curvature_cmd_checks(int desired_curvature, bool steer_control_enabled,
                                const CurvatureSteeringLimits limits);
bool steer_curvature_cmd_checks_average(int desired_curvature, bool steer_control_enabled,
                                        const CurvatureSteeringLimits limits);
