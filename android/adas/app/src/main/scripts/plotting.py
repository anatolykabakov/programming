...
def plot_trajectory(
    ...
    plt.plot(x_odom_array, y_odom_array, "b-", linewidth=2, alpha=0.7, label="Одометрия (Steering)")
    plt.scatter(
        x_odom_array[0], y_odom_array[0], color="green", s=150, label="Начало", zorder=5, marker="o"
    )
    plt.scatter(
        x_odom_array[-1], y_odom_array[-1], color="red", s=150, label="Конец", zorder=5, marker="s"
    )
    ...
    if x_fusion is not None and y_fusion is not None and len(x_fusion) > 0:
        ...
        plt.scatter(
            x_fusion_array[0], y_fusion_array[0], color="green", s=100, zorder=4, marker="*"
        )
        plt.scatter(
            x_fusion_array[-1], y_fusion_array[-1], color="red", s=100, zorder=4, marker="*"
        )
    ...
    if x_gps_fusion is not None and y_gps_fusion is not None and len(x_gps_fusion) > 0:
        ...
        plt.scatter(
            x_gps_fusion_array[0], y_gps_fusion_array[0], color="green", s=120, zorder=5, marker="P"
        )
        plt.scatter(
            x_gps_fusion_array[-1], y_gps_fusion_array[-1], color="red", s=120, zorder=5, marker="P"
        )
    ...
    if x_gps is not None and y_gps is not None and len(x_gps) > 0:
        ...
        y_gps_array = np.array(y_gps)
        plt.plot(
            x_gps_array, y_gps_array, "r--", linewidth=2, alpha=0.6, label="GPS (Ground Truth)"
        )
