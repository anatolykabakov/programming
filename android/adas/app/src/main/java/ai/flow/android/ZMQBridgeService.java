...
public class ZMQBridgeService extends Service {
    ...
    private static final Map<String, String> DEFAULT_PUB_TOPICS = new HashMap<String, String>() {{
        put("can/tx", "tcp://127.0.0.1:5564");                 // Joystick CAN commands
    ...
    private static final Map<String, String> DEFAULT_SUB_TOPICS = new HashMap<String, String>() {{
        put("can/rx", "tcp://127.0.0.1:5563");        // CAN RX data to Java
        put("panda/health", "tcp://127.0.0.1:5565");  // Panda health status to Java
    }};
    ...
    private void processCANRxMessage(ai.flow.android.Can.CANData canData) {
        try {
            for (int i = 0; i < canData.getFramesCount(); i++) {
                ai.flow.android.Can.CANFrame frame = canData.getFrames(i);
    ...
    private void processPandaHealthMessage(ai.flow.android.Panda.PandaHealth zmqMsg) {
        try {
